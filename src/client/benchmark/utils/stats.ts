 ...
 */
export const formatStats = (stats: RunnerResult, runs: number, count: number, updates: number, operationLabel: string): string => {
 ...
 */
export const computeSeed = (config: { count: number; runs: number; updates: number; mutateCount: number }): number => {
