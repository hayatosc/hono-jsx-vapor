...
- 要素数と試行回数は画面上の入力で変更可能（デフォルト 1,000 件、5 回）
- 更新は `requestAnimationFrame` を2回待って paint まで含めて計測し、同じシードのデータ／更新パターンを両者に渡します（run-all は実行順を毎回交互に切り替え）
- 初回マウントと状態更新を分けて集計し、更新は平均・中央値・p95・標準偏差を表示します（アンマウントも計測できる場合は平均を表示）
